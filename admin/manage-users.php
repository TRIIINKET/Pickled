...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!pickled_validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        } elseif ($action === 'delete_user' && $id && $id !== (int) $_SESSION['user']['id']) {
            $successMsg = $adminService->deleteUser($id, (int) $_SESSION['user']['id']) ? 'User deleted.' : '';
            $errorMsg = $successMsg ? '' : 'Failed to delete user.';
            $userId = 0;
        }
    }
}
...
$dashboardNav = [
    ...
    ['type' => 'group', 'label' => 'Programs', 'href' => 'manage-events.php', 'key' => 'events', 'icon' => 'target', 'children' => [['Social Play', 'manage-events.php?program=social-play', ''], ['Private Sessions', 'manage-events.php?program=private', '']]],
    ['type' => 'group', 'label' => 'Content', 'href' => 'notifications.php', 'key' => 'content', 'icon' => 'image', 'children' => [['Photos', 'notifications.php?content=photos', ''], ['Catalogs', 'notifications.php?content=catalogs', '']]],
...
<div class="admin-app-shell">
    <aside class="admin-sidebar">
        ...
        <div class="admin-sidebar-user"><div class="admin-avatar"><?php echo htmlspecialchars(strtoupper(substr($adminName, 0, 1))); ?></div><div><strong><?php echo htmlspecialchars($adminName); ?></strong><span>Super Admin</span></div></div>
    </aside>

    <main class="admin-dashboard-main users-main">
        <header class="admin-topbar">
            <div><h1><?php echo htmlspecialchars($pageTitle); ?></h1></div>
            <div class="admin-topbar-actions"><button class="admin-date-pill" type="button"><?php echo users_icon($icons, 'calendar'); ?><span><?php echo htmlspecialchars($todayLabel); ?></span></button><a class="admin-notification" href="<?php echo pickled_admin_url('notifications.php'); ?>"><?php echo users_icon($icons, 'bell'); ?></a><div class="admin-profile"><div class="admin-avatar"><?php echo htmlspecialchars(strtoupper(substr($adminName, 0, 1))); ?></div><div><strong><?php echo htmlspecialchars($adminName); ?></strong><span>Super Admin</span></div><form method="post" action="<?php echo pickled_admin_url('admin-logout.php'); ?>"><input type="hidden" name="csrf_token" value="<?php echo $logoutCsrf; ?>"><button type="submit">⌄</button></form></div></div>
        </header>

        <section class="users-hero">
            <div><h2><?php echo htmlspecialchars($pageTitle); ?></h2><p><?php echo $roleFilter === 'coach' ? 'Manage registered Pickled coaches and training activity.' : 'Manage all registered Pickled members and track activity.'; ?></p></div>
        ...
        <section class="users-layout">
            <div class="users-workspace">
                <section class="users-stat-grid">
                    <article class="user-stat pink"><div><?php echo users_icon($icons, 'users'); ?></div><span>Total <?php echo $roleFilter === 'coach' ? 'Coaches' : 'Players'; ?></span><strong><?php echo number_format($totalUsers); ?></strong><small>All members</small></article>
